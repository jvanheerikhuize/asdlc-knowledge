...
def _alias_slug(text: str) -> str:
    """URL-safe slug for an alias, matching MkDocs' directory-URL style."""
    import re
    s = re.sub(r"[^\w\s-]", "", str(text).strip().lower())
    return re.sub(r"[\s_]+", "-", s).strip("-")


def _alias_redirects(m: dict) -> dict[str, str]:
    """alias-slug doc path -> real page doc path, from each page's `aliases:`
    frontmatter. Powers mkdocs-redirects so a URL for an old/alternate name
    (e.g. /karpathy/) 301s to the canonical page. Collisions with a real page
    id, or a second alias claiming the same slug, are skipped (first wins) so a
    ...
    for p in pages:
        aliases = p.frontmatter.get("aliases") or []
        if isinstance(aliases, str):
            aliases = [aliases]
        for alias in aliases:
            slug = _alias_slug(alias)
            if not slug or slug in ids or slug in redirects:
                continue
            folder = folders.get(p.type, p.type or "page")
            redirects[f"{slug}.md"] = f"{folder}/{p.id}.md"
    return redirects
...
def build_mkdocs(m: dict) -> str:
    ...
    # Redirect alias URLs to their canonical page (mkdocs-redirects). Omit the
    # plugin entirely when there are no aliases so the config stays minimal.
    redirects = _alias_redirects(m)
    redirects_block = ""
    if redirects:
        maps = "\n".join(f"        {json.dumps(k)}: {json.dumps(v)}"
                         for k, v in sorted(redirects.items()))
        redirects_block = (
            "\n  # Redirect alias URLs (from pages' `aliases:` frontmatter) to the\n"
            "  # canonical page, so an old/alternate name resolves by URL too.\n"
            "  - redirects:\n      redirect_maps:\n" + maps)
...
extra_css:
  # Icons for the nav page-status markers (see `extra.status` below). Generated
  # into wiki/assets/status.css by `python tools/kb.py viz`.
  - assets/status.css
extra:
  # Page-status markers: Material renders a small icon + hover tooltip next to a
  # nav entry whose `status:` frontmatter matches a key here. The text is the
  # tooltip; the icon comes from extra_css. Keys mirror the `status` field that
  # scaffold/ingest set and `verify` flips (kept honest by `kb lint`).
  status:
    verified: Verified against its cited sources
    draft: Draft — not yet fact-checked
plugins:
  - search
...
# Icons for the nav page-status markers. Material's `.md-status::after` masks
# `var(--md-status--<name>)` over `currentcolor`; we define that var per custom
# status (verified -> check-circle, draft -> pencil) the same way the theme does
# for its built-in `new`/`deprecated`. The `::after` rules point each custom
# status class at its var (the theme only wires this for its own names).
_STATUS_CSS = """/* GENERATED by tools/viz.py. Do not hand-edit. */
/* Nav page-status icons for the `status:` frontmatter values (see extra.status
   in mkdocs.yml). Regenerate with `python tools/kb.py viz`. */
:root {
  --md-status--verified: url('data:image/svg+xml;charset=utf-8,\
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">\
<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2\
m-2 15-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8z"/></svg>');
  --md-status--draft: url('data:image/svg+xml;charset=utf-8,\
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">\
<path d="M20.71 7.04c.39-.39.39-1.04 0-1.41l-2.34-2.34c-.37-.39-1.02-.39-1.41 0\
l-1.84 1.83 3.75 3.75M3 17.25V21h3.75L17.81 9.93l-3.75-3.75z"/></svg>');
}
.md-status--verified::after { mask-image: var(--md-status--verified);
  -webkit-mask-image: var(--md-status--verified); }
.md-status--draft::after { mask-image: var(--md-status--draft);
...
def main() -> int:
    ...
    # Icons for the nav page-status markers (referenced by extra_css in mkdocs.yml).
    css_path = KB_ROOT / m["paths"]["wiki"] / "assets" / "status.css"
    css_path.parent.mkdir(parents=True, exist_ok=True)
    css_path.write_text(_STATUS_CSS)
    print(f"generated {css_path.relative_to(KB_ROOT)}")
